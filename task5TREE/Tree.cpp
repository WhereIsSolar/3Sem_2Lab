...
static void printRec(RBNode* node, int indent = 0) {
    ...
    cout << string(indent, ' ');
    cout << (node->color == RED ? "R:" : "B:") << node->value << "\n";
...
void readFromFile(RBTree &tree, const string &filename) {
...
static void writeRec(RBNode* node, ofstream &out) {
    if (!node) return;
...
void writeToFile(RBTree &tree, const string &filename) {
